...
public class RisultatoTest extends Activity implements View.OnClickListener {
    ...
    public void salvataggio() {
        ...
        if (save_session != "si") {
            ...
            if (doc == "0") {
                ...
                // direttamente
                p = new AccessoDatabase("storeresults&username=" + username + "&useremail=" +
                        email + "&password=" + password + "&idapp=" + "AcuityTest" +
                        "&result=" + risultato_test);
                //salvato = conn.saveResults(username, email, "AcuityTest", ts, RisultatoTest);
                editor.putString("salvato", "si");
                Log.v(TAG, "SALVATOOO:");

            } else {

                //sto accedendo con il profilo del dottore
                if (idPaziente.compareTo("null") == 0) {
                    ...
                    p = new AccessoDatabase(prova);
                    // salvato = conn.storeResultsFromDoc(email, password, username, emailuser, "AcuityTest", ts, RisultatoTest);
                    editor.putString("salvato", "si");
                    ...
                    p = new AccessoDatabase(prova);
                    //salvato = conn.storeResultsNotRegistered(email, password, idPaziente, "AcuityTest", ts, RisultatoTest);
                    editor.putString("salvato", "si");
                    Log.v(TAG, "SALVATOOO:" + prova);
